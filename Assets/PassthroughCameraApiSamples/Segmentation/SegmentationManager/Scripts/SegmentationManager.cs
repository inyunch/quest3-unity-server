...
namespace PassthroughCameraSamples.Segmentation
{
    ...
    public class SegmentationManager : MonoBehaviour
    {
        ...
        [SerializeField] private SegmentationSpawnMarkerAnim m_spawnMarker;

        [SerializeField] private SegmentationInferenceUiManager m_uiInference;
        [Space(10)]
        public UnityEvent<int> OnObjectsIdentified;

        private readonly List<SegmentationSpawnMarkerAnim> m_spawnedEntities = new();
        ...
        {
            var newCount = 0;
            foreach (SegmentationInferenceUiManager.BoundingBoxData box in m_uiInference.m_boxDrawn)
            {
                if (!HasExistingMarkerInBoundingBox(box))
                {
                    ...
                    marker.GetComponent<SegmentationSpawnMarkerAnim>().SetYoloClassName(box.ClassName);
            ...
            bool HasExistingMarkerInBoundingBox(SegmentationInferenceUiManager.BoundingBoxData box)
            {
